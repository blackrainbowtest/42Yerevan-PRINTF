...
# Բոնուսով աշխատելու համար՝

## Անհրաժեշտ է որոշել սեփական string parcing ալգորիթմը, անհրաժեշտ տվյալները ստանալու համար։

### printf flags

| Դրոշներ | Նկարագրություն | Օրինակ |
|------|--------------|--------|
| `-`  | Տեքստը դնում է ձախից, լրացուցիչ տարածքները լրացվում են աջից | `%-5d` → `"42   "` |
| `+`  | Դրական թվերի առաջ ավելացնում է `+` նշան | `%+d` → `+42` |
| ` ` (դատարկություն) | Դրական թվերի առաջ ավելացնում է բացատ (եթե չկա `+`) | `%" d"` → `" 42"` |
| `#`  | 8-ական (օկտալ) և 16-ական (հեքս) թվերի համար ավելացնում է `0` կամ `0x` | `%#x` → `0x2a`, `%#o` → `052` |
| `0`  | Բաց թողնված տարածքները լրացնում է `0`-երով (եթե չկա `-`) | `%05d` → `"00042"` |
